...
### 各章を進めるときの学習アプローチ
- 3章 制御構文: 既存のワークアウトPOSTバリデーションをRubyで書き直し、if/elsifの分岐を削れるか意識する。
- 4–5章 配列操作: `repsNumber` 整形やStrava活動リストを題材に、`map/reduce` で副作用を減らす練習をする。
- 6章 ハッシュ: JWTペイロードやCORS許可リストをRubyのHashで表現し、`fetch` で必須キーを強制する癖を付ける。
- 7章 メソッド: 引数のデフォルト値とキーワード引数を活用し、Stravaトークン暗号化のような小機能を短いメソッドに分割する。
- 8章 クラス: SmallWins計算をクラス化し、`initialize` で依存を注入する形にリファクタする。
- 9章 モジュール: 日付ヘルパーや定数群をモジュール化し、名前空間衝突を避けつつ`include`/`extend`の違いを意識する。
- 10–11章 進んだ操作: 例外クラスを自作し、Strava同期でのリトライ/バックオフ処理をEnumerableと例外で整理する。
